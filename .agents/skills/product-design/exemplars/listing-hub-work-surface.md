...
- **Decision:** Wide operational layout: photos (sectioned by job), Finish with AI, inline listing fields, sticky QR rail for Phone Companion. Header is typography (platform · status, title, job sentence) plus `← All listings`. AI is a section with a disabled state when there are no photos. Private tag shots sit in `<details>`.
